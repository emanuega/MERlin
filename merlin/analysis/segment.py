...
class AssignCellFOV(analysistask.AnalysisTask):
    ...
    def _get_fov_positions(self):
        xDim = self.dataSet.micronsPerPixel * self.dataSet.imageDimensions[0]
        yDim = self.dataSet.micronsPerPixel * self.dataSet.imageDimensions[1]

        tiledPositions = self.dataSet.positions.copy(deep=True)
        tiledPositions['minx'] = tiledPositions['X']
        tiledPositions['maxx'] = tiledPositions['X'] + (xDim)
        tiledPositions['miny'] = tiledPositions['Y']
        tiledPositions['maxy'] = tiledPositions['Y'] + (yDim)
        tiledPositions['centerX'] = (tiledPositions['minx'] + tiledPositions['maxx']) / 2
        tiledPositions['centerY'] = (tiledPositions['miny'] + tiledPositions['maxy']) / 2
        boxes = [geometry.box(x[0], x[1], x[2], x[3]) for x in
                 tiledPositions.loc[:, ['minx', 'miny', 'maxx', 'maxy']].values]

        return tiledPositions, boxes

    def _construct_fov_tree(self,tiledPositions: pandas.DataFrame, fovIntersections: List):
        return cKDTree(data=tiledPositions.loc[fovIntersections, ['centerX', 'centerY']].values)

    # def _get_unique_fov_region(self,currentFOV: int, allFOVBoxes: List):
    #     FOVintersections  = sorted([x for x in range(len(allFOVBoxes))
    #                                 if allFOVBoxes[currentFOV].intersects(allFOVBoxes[x])])
    #
    #     uniqueRegion = allFOVBoxes[currentFOV]
    #     for intersectingFOV in FOVintersections:
    #         if intersectingFOV == currentFOV:
    #             pass
    #         else:
    #             uniqueRegion = uniqueRegion.difference(allFOVBoxes[intersectingFOV])
    #     return uniqueRegion

    def _intial_clean(self,currentFOV: int):
        # I'm assuming that if a cell is within the unique portion of a fov it cannot overlap
        # any other cells, if this constraint isn't true there will be overlapping cells
        currentCells = self.segmentTask.get_feature_database().read_features(currentFOV)
        cleanCells = [len(cell.get_bounding_box()) == 4 for cell in currentCells]
        currentCells = np.take(currentCells, np.where(cleanCells))[0].tolist()
        return currentCells

    def _secondary_assignments(self,currentFOV: int, secondaryAssignmentDict: Dict):
        currentCells = self._intial_clean(currentFOV)

        secondaryAssignments = [[x,secondaryAssignmentDict[currentFOV][x.get_feature_id()]]
                                for x in currentCells
                                if x.get_feature_id() in secondaryAssignmentDict[currentFOV]]
        return secondaryAssignments

    def _construct_spatial_tree(self,tree: rtree.index.Index, cells: List, idToNum: Dict):
        for element in cells:
            tree.insert(idToNum[element.get_feature_id()], element.get_bounding_box(),obj = element.get_fov())
    ...
    def _run_analysis(self):
        ...
        allFOVs = self.dataSet.get_fovs()[:20]
        tiledPositions, allFOVBoxes = self._get_fov_positions()
        numToID = dict()
        idToNum = dict()
        idTracking = 0
        ...
        for currentFOV in self.dataSet.get_fovs():
        ...
        for currentFOV in allFOVs:
            ...
            for cell in currentCells:
                overlappingCells = idx.intersection(cell.get_bounding_box(),objects = True)
                toCheck = []
                for c in overlappingCells:
                    xmin,ymin,xmax,ymax = c.bbox
                    toCheck.append([numToID[c.id], c.object, xmin, ymin, xmax, ymax])
                if len(toCheck) == 0:
                    # I dont know if this case will come up but want to guard against this, it implies
                    ...
                    # keep it and assign it to an fov based on the fov centroid it's closest to
                    if len(toCheck) == 1 and cell.get_feature_id() == toCheck[0][0]:
                        xCenter = (toCheck[0][2] + toCheck[0][4]) / 2
                        yCenter = (toCheck[0][3] + toCheck[0][5]) / 2
                        [d,i] = fovTree.query(np.array([xCenter,yCenter]))
                        assignedFOV = tiledPositions.loc[FOVintersections, :].index.values.tolist()[i]
                        secondaryAssignments[currentFOV][toCheck[0][0]] = assignedFOV
                    # I dont know if this case will come up but want to guard against this, it implies
                    # there was an error in the original tree construction
                    elif len(toCheck) == 1 and not cell.get_feature_id() == toCheck[0][0]:
                        print('encountered unexpected overlap between {} and {}'.format(cell.get_feature_id(),
                                                                                        toCheck[0][0]))
                    # If a cell overlaps at least one other cell first check if all cells are closest to
                    # a particular FOV centroid, then if a cell boundary was determined from that fov
                    # keep it, else choose a cell at random. If overlapping cells are closest to different
                    # fov centroids, again keep one at random
                    elif len(toCheck) >= 2:
                        toCheckDF= pandas.DataFrame(toCheck,
                                                    columns = ['cell ID','initial FOV','xmin','ymin','xmax','ymax'])
                        toCheckDF['centerX'] = (toCheckDF.loc[:, 'xmin'] + toCheckDF.loc[:, 'xmax']) / 2
                        toCheckDF['centerY'] = (toCheckDF.loc[:, 'ymin'] + toCheckDF.loc[:, 'ymax']) / 2
                        [d, i] = fovTree.query(toCheckDF.loc[:,['centerX','centerY']], k=1)
                        assignedFOV = np.array(tiledPositions.loc[FOVintersections, :].index.values.tolist())[i]
                        toCheckDF['assigned FOV'] = assignedFOV
                        if len(np.unique(assignedFOV)) == 1:
                            if len(toCheckDF[toCheckDF['initial FOV'] == np.unique(assignedFOV)[0]]) > 0:
                                selected  = toCheckDF[toCheckDF['initial FOV'] == np.unique(assignedFOV)[0]].sample(n=1)
                                selectedFOV = selected.loc[:, 'assigned FOV'].values.tolist()[0]
                                selectedCellID = selected.loc[:, 'cell ID'].values.tolist()[0]
                                secondaryAssignments[currentFOV][selectedCellID] = selectedFOV
                            else:
                            ...
                            matching = toCheckDF[toCheckDF['initial FOV'] == toCheckDF['assigned FOV']]
                            if len(matching) == 1:
                                selectedFOV = matching.loc[:, 'assigned FOV'].values.tolist()[0]
                                selectedCellID = matching.loc[:, 'cell ID'].values.tolist()[0]
                                secondaryAssignments[currentFOV][selectedCellID] = selectedFOV
                            elif len(matching) == 0:
                                selected = toCheckDF.sample(n=1)
                                selectedFOV = selected.loc[:, 'assigned FOV'].values.tolist()[0]
        ...
        for currentFOV in allFOVs:
            secondaryCellAssignments = self._secondary_assignments(currentFOV, secondaryAssignments)
            allFOV = list(set([x[1] for x in secondaryCellAssignments]))
            for f in allFOV:
                cellsInFOV = [x[0] for x in secondaryCellAssignments if x[1] == f]
